...
**Livré et testé** (`npm run test` : 687 tests, 19 fichiers) :
...
- **Prêts en cours** (page `/prets`) : immobilier, rénovation (dont éco-PTZ à 0 %), consommation, autre ; échéancier calculé, capital restant dû, mensualités (assurance comprise), intérêts restants, dates de fin, évolution à 1, 2, 3 et 5 ans, calcul d'une mensualité depuis une durée ; carte sur le tableau de bord. Aucun batch : les échéances dont la date est passée sont considérées payées à chaque affichage ; le capital saisi reste le point de départ (jamais à mettre à jour chaque mois) et le formulaire du prêt montre le capital restant dû aujourd'hui, les échéances restantes et le reste à payer.
...
- **Fonds réservés d'un prêt** (dans le formulaire du prêt) : part déjà versée sur un ou plusieurs comptes mais pas encore à sa destination finale (ex. prêt travaux crédité en attendant de payer l'artisan) ; note et date facultatives, une ou plusieurs allocations (compte + montant). Catégorie « Réservé » distincte du « Bloqué » (fonds bloqués par une règle bancaire) ; exclue du déblocable et du dépensable partout où ils apparaissent (bannière, comptes, projections). Retirée manuellement une fois la somme payée à sa destination (aucune détection automatique) ; supprimer le compte concerné détache l'allocation.
...
* **`ProjectionEngine` :**
  ...
  - Projection de 61 points (0 = aujourd'hui, jusqu'à 60 mois), horizons 12/24/36/60 : intérêts composés chaque mois (taux/12 sur le solde d'ouverture, arrondis au centime, aucun sur un solde ≤ 0), versement net moyen ajouté en fin de mois. Chaque point porte `balance`, `locked`, `reserved` et `available` (= balance − locked − reserved). `locked` est calculé en premier et plafonné au solde ; `reserved` (fonds réservés d'un prêt, voir `LoanEngine`) est ensuite plafonné à ce qu'il reste après `locked`, pour ne jamais compter deux fois le même centime. `projectAccount`/`projectPortfolio` acceptent en dernier paramètre optionnel les prêts (`loans`), sans quoi `reserved` vaut 0.
